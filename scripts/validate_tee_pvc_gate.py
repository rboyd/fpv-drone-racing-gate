...
O=Path(__file__).resolve().parents[1]/'output/tee_pvc_gate';b=json.loads((O/'BOM.json').read_text());report={'geometry':{},'plates':{},'slices':{},'gates':{}}
for p in b['parts']:
...
for kind,g in b['gates'].items():
 ...
 assert set(g['counts'])=={'ELBOW','GUY_ELBOW','TEE','CROSS','MAG_SLEEVE'}
 ...
 assert sum(q['code']=='GUY_ELBOW' for q in g['nodes'].values())==2
 assert all(q['xy'][1]==max(g['y_grid_mm']) for q in g['nodes'].values() if q['code']=='GUY_ELBOW')
...
import hashlib
previous=O.parent/'orthogonal_pvc_gate'
