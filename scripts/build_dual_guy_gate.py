"""Dual front/rear guy elbow with diagonal twin-eye ear and two edge magnet pads."""
from pathlib import Path
exec(Path(__file__).with_name('build_a1_full_size.py').read_text().split('# Registered channel splice:')[0],globals())
import ast,csv
OUT=ROOT/'output/dual_guy_gate'
...
MAGS={'ELBOW':[(-OFFSET,-OFFSET)],'DUAL_GUY_ELBOW':[(-OFFSET,15),(15,-OFFSET)],'TEE':[(-OFFSET,-OFFSET),(OFFSET,-OFFSET)],'CROSS':[(sx*OFFSET,sy*OFFSET) for sx in [-1,1] for sy in [-1,1]],'MAG_SLEEVE':[(0,OFFSET)]}
...
for code,angles in PORTS.items():
 ...
 desc={'ELBOW':'Two-port elbow for bottom outside corners','DUAL_GUY_ELBOW':'Top elbow with reinforced diagonal ear, two independent 10.5 mm front/back through-eyes and separate top/side magnetic pads','TEE':'Three-way tee, native ports +X, +Y, -X; two magnetic pads on missing-port side','CROSS':'Four-way cross for the inner grid intersections'}[code]
...
footer('Two top/side magnetic pads replace the corner pad. PVC bore stays 33.5 mm, with the same 30 mm engagement.')
...
header('03','Front guy access without puncturing paper','The diagonal ear projects beyond the paper corner. Each magnetic pad sits 18 mm inside one edge and 90 mm along it.')
...
header('05','Two revised top corners / same gate face','2700 mm square, 1480.8 mm opening, 609.6 mm bands. Magnet total is now 74 pairs / 148 individual magnets.')
footer('Five production part types and 64 pieces. Replace the two old guy elbows; keep the other parts and PVC cuts.')
setup('06_STACKED_REAR',(4,7,4.8),(0,0,2.80),10.2,False);render_gate(GATES['split_s'],False)
header('06','The same top corners extend to Split-S','2700 x 4790.4 mm paper outline. Two dual-guy elbows, 118 magnet pairs, and unchanged pipe and fitting quantities.')
...
bpy.context.window.scene=bpy.data.scenes['01_DUAL_GUY_PART'];bpy.ops.file.pack_all();bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'Dual_Guy_Gate.blend'))
