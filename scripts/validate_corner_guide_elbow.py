...
O=Path(__file__).resolve().parents[1]/'output/corner_guide_elbow';b=json.loads((O/'BOM.json').read_text());report={'geometry':{},'plates':{},'slices':{},'gates':{}}
costs=json.loads((O.parents[1]/'docs/material_costs.json').read_text());petg_per_g=costs['petg_pack_usd']/(costs['petg_pack_kg']*1000);magnet_each=costs['magnet_pack_usd']/costs['magnet_pack_quantity']
for p in b['parts']:
...
for kind,g in b['gates'].items():
 assert g['internal_cords']==0 and g['diagonal_PVC']==0
 assert set(g['counts'])=={'ELBOW','DUAL_GUY_ELBOW','TEE','CROSS','MAG_SLEEVE'}
 ...
 # Verify every node has exactly the needed ports, including rotated tees.
 for node in g['nodes'].values():
  x,y=node['xy'];expected=[]
  for e in g['edges']:
   aa=g['nodes'][e['a']]['xy'];zz=g['nodes'][e['b']]['xy']
   if node['xy']==aa:expected.append(round(math.degrees(math.atan2(zz[1]-y,zz[0]-x)))%360)
   elif node['xy']==zz:expected.append(round(math.degrees(math.atan2(aa[1]-y,aa[0]-x)))%360)
  actual=[(r+node['angle'])%360 for r in b['ports_local_degrees'][node['code']]]
  assert sorted(expected)==sorted(actual),(kind,node,expected,actual)
 assert sum(q['code']=='DUAL_GUY_ELBOW' for q in g['nodes'].values())==2
 assert all(q['xy'][1]==max(g['y_grid_mm']) for q in g['nodes'].values() if q['code']=='DUAL_GUY_ELBOW')
 ...
 for code,xy,rot in g['placements']:
  ...
  local=b['magnet_pockets_local_mm'][code]
 ...
 report['gates'][kind]={**totals,'printed_pieces':sum(made.values()),'unique_production_types':len(made),'plate_runs':sum(g['print_queue'].values()),'pipe_sticks':g['PVC_sticks'],'pipe_cost_usd':g['PVC_cost_usd'],'PETG_cost_usd':round(totals['grams']*petg_per_g,2),'magnet_cost_usd':round(g['magnet_pairs']*2*magnet_each,2),'priced_materials_usd':round(g['PVC_cost_usd']+totals['grams']*petg_per_g+g['magnet_pairs']*2*magnet_each,2),'frame_PVC_plus_PETG_usd':round(g['PVC_cost_usd']+totals['grams']*petg_per_g,2),'magnet_pairs':g['magnet_pairs'],'largest_pipe_cut_mm':max(e['length_mm'] for e in g['edges']),'sleeves':sleeve_checks}
checks=json.loads((O/'geometry_checks.json').read_text())
for kind in b['gates']:
 g=checks[kind]
 assert g['only_orthogonal_PVC'] and not g['unexpected_printed_vertices_outside_paper'] and not g['magnet_centers_outside_paper']
 assert g['exposed_diagonal_ear_vertex_count']>0
 assert len(g['eye_clearance'])==4 and all(e['entire_hole_outside_paper_outline'] for e in g['eye_clearance'])
assert checks['local_cord_routing']['no_mesh_or_paper_intersection']
...
top_pockets=set()
for code,xy,r in single['placements']:
 if abs(xy[1]-top)>.01:continue
 angle=math.radians(r)
 for x,y in b['magnet_pockets_local_mm'][code]:top_pockets.add((round(xy[0]+x*math.cos(angle)-y*math.sin(angle),2),round(xy[1]+x*math.sin(angle)+y*math.cos(angle),2)))
assert all((round(x,2),round(y+(shift if (round(x,2),round(y,2)) in top_pockets else 0),2)) in new_magnets for x,y in single['magnets'])
report['upgrade']['intact_top_row_and_all_populated_pockets_reused']=True
assert b['PVC_bore_mm']==33.5
import hashlib
previous=O.parent/'tee_pvc_gate'
assert (O/'printable/MAG_SLEEVE.stl').read_bytes()==(previous/'printable/MAG_SLEEVE.stl').read_bytes(),'Accepted sleeve changed'
for code in ['ELBOW','TEE','CROSS','MAG_SLEEVE']:
 assert (O/'printable'/(code+'.stl')).read_bytes()==(previous/'printable'/(code+'.stl')).read_bytes(),('Unrequested part changed',code)
report['unchanged_parts']=['ELBOW','TEE','CROSS','MAG_SLEEVE']
report['accepted_sleeve_unchanged']=True
report['PVC_bore_mm']=33.5
