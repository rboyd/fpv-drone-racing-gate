...
assert sum(produced.values())==560 and len(produced)==8
report['kit_totals']={k:round(sum(report['slices'][n][k]*r for n,r in bom['plate_runs'].items()),2) for k in ['grams','minutes']}
report['kit_totals_without_optional_pads']={k:round(sum(report['slices'][n][k]*r for n,r in bom['plate_runs'].items() if not n.startswith('Q_PAPER_PAD')),2) for k in ['grams','minutes']}
report['test_runs']={'T01_BRANCH':4,'T02_RUNG':1,'T05_SINGLE_PLAIN':2,'T03_BAY_SLEEVE':1}
...
print('PASS: 11 manifold masters (8 production types + 3 gauges), all plates within A1 Mini, no slice warnings, exact 560 piece production queue and assembly counts.')
